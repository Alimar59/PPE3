...
function obtenirDetailVisiteur($idCnx, $unId) {
    ...
    $requete = "select id, nom, prenom,comptable from utilisateur where id='" . $unId . "'";
...
function existeFicheFraisComptable($idCnx, $unMois, $unIdVisiteur) {
    $unMois = filtrerChainePourBD($unMois);
    $requete = "select idVisiteur from FicheFrais where mois='" . $unMois . "'";
    $idJeuRes = mysqli_query($idCnx,$requete);  
    $ligne = false ;
    if ( $idJeuRes ) {
        $ligne = mysqli_fetch_assoc($idJeuRes);
...
function obtenirReqMoisFicheFraisComptable($unIdVisiteur) {
 ...
 */
function verifierInfosConnexion($idCnx, $unLogin, $unMdp,$unCaptcha) {
